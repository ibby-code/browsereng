...
class TestUrl(unittest.TestCase):
    ...
    def test_http_redirect(self, mock_socket_ctr):
        ...
        mock_socket.send.assert_has_calls([call(request_one.encode("utf8")), call(request_two.encode("utf8"))])
        self.assertEqual(response, ("Body text", 0))
    ...
    def test_http_redirect_new_socket(self, mock_socket_ctr):
        ...
        mock_socket.connect.asset_has_calls([call(('google.com', 80)), call(('google.com', 4229))])
        self.assertEqual(response, ("Body text", 0))
    ...
    def test_http_redirect_limit(self, mock_socket_ctr):
        redirect_url = "http://google.com/somethingelse"
        http_r = f"HTTP/1.1 301 MovedPermanentely\r\nLocation: {redirect_url}\r\n\r\nBody text"
        get_mock_socket(mock_socket_ctr, [http_r, http_r, http_r, http_r, http_r, http_r, HTTP_RESPONSE])
 
        test_url = "http://google.com/something" 
        u = url.URL(test_url)
        response = u.request()

        self.assertEqual(response, (f"Redirect loop detected! Last redirect is to :{redirect_url}", 0))
